"""
FastAPI Application — Customer Success Digital FTE (Stage 2)

Entry point for the Stage 2 backend service.
...
from src.mcp.tool_registry import init_tools
from src.services.knowledge_service import seed_all
...
async def lifespan(app: FastAPI):
    ...
    """
    logger.info("=== Nexora Customer Success Agent — Stage 2 Starting ===")
    ...
    yield
    logger.info("=== Nexora Customer Success Agent — Stage 2 Shutting Down ===")
...
app = FastAPI(
    ...
    description=(
        "Stage 2 backend service for the AI Customer Success agent. "
        "Handles inbound support messages from Email, WhatsApp, and Web Form channels. "
        "Processes messages through an intelligent agent workflow with database persistence, "
        "escalation detection, knowledge base search, and ticket creation.\n\n"
